...
def save_data(request):
    if request.method == 'GET':
        ...
        input1_data = request.GET.get('input1_data', '')
        ...
        #print(f"Received input_data: {input1_data}, {input2_data}, picker_value1: {picker_value1}, picker_value2: {picker_value2}, {picker_date}, {picker_time}, datacode: {datacode}")
        timestamp = datetime.datetime.now().replace(microsecond=0).strftime("%Y年%m月%d日%H:%M:%S")
        ...
        try:
            ...
            # 设置缓存
            cache.set('input1_data', input1_data, timeout=300)
