Page({
  ...
  loadWordsBatch() {
    ...
    // 从缓存中获取 CET4 单词
    const cachedWordsGrouped = wx.getStorageSync('cet6WordsGrouped') || {};
    ...
    // 更新页面数据
    this.setData({
      cet6WordsGrouped: cachedWordsGrouped,
  ...
  onClick(){
    console.log("点击搜索")
    const {searchValue, cet6WordsGrouped, alphabet } = this.data;
    const searchValueLower = searchValue.toLowerCase();
    let foundWord = '';

    // 查找匹配的单词
    for (let letter of alphabet) {
      const words = cet6WordsGrouped[letter] || [];
    ...
    if (foundWord) {
      ...
      wx.createSelectorQuery().select(`#${foundWord}`).boundingClientRect((rect) => {
        if (rect) {
          wx.pageScrollTo({
            scrollTop: rect.top + this.data.windowHeight * (-0.05),  // 调整位置以便可见
            duration: 100  // 滚动动画时间
          });
        }
      }).exec();
